...
#         utils.setup_logging(os.path.join(cfg.LOG_DIR, "log.txt"))
        logging.basicConfig(filename=os.path.join(cfg.LOG_DIR, "log.txt"),level=logging.INFO)
        ...
        if self.use_gpu:
            ...
            logging.info('Number of GPU available: %d' % torch.cuda.device_count())
            self.model.cuda()
            self.priors.cuda()
    ...
    def resume_checkpoint(self, resume_checkpoint):
        if resume_checkpoint == '' or not os.path.isfile(resume_checkpoint):
            logging.info(("=> no checkpoint found at '{}'".format(resume_checkpoint)))
            return False
        logging.info(("=> loading checkpoint '{:s}'".format(resume_checkpoint)))
        checkpoint = torch.load(resume_checkpoint)

        # logging.info("=> Weigths in the checkpoints:")
        # logging.info([k for k, v in list(checkpoint.items())])
        ...
        if resume_scope != '':
            ...
            logging.info("TRAIN.RESUME_SCOPE is NULL, recover all parameter from provide model file")
        ...
        unresume_dict = [x for x in unresume_dict if 'num_batches_tracked' not in x]
        unresume_dict.sort()
    ...
    def trainable_param(self, trainable_scope):
        ...
        for module in trainable_scope.split(','):
            if hasattr(self.model, module):
                # logging.info(getattr(self.model, module))
    ...
    def test_model(self):
        self.cfg.TRAIN.RESUME_SCOPE = ''  # force reload all parameter
        previous = self.find_previous()
        if previous:
    ...
    def visualize_epoch(self, model, data_loader, priorbox, writer, epoch, use_gpu):
        ...
        images = preproc(image, anno)[0].unsqueeze(0)
        if use_gpu:
            images = images.cuda()
    ...
    def export_graph(self):
        ...
        dummy_input = torch.randn(1, 3, cfg.MODEL.IMAGE_SIZE[0], cfg.MODEL.IMAGE_SIZE[1]).cuda()
        # Export the model
